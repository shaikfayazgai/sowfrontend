...
/**
 * Platform-admin forced first-login password reset — same pattern as enterprise.
 * Reached when a freshly-provisioned admin (default password) signs in
 * (`must_change_password` → `session.user.requiresPasswordChange`).
 * Flow: set new password → POST /api/admin/auth/change-password → re-auth → dashboard.
 */
...
export default function AdminResetPasswordPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      router.replace("/admin/login?reason=password_changed");
    } catch {
  ...
  return (
    <LoginShell>
      ...
      <form onSubmit={onSubmit} className="space-y-4">
        <AuthField label="New password" htmlFor="admin-reset-pw">
        ...
        <AuthField label="Confirm password" htmlFor="admin-reset-pw2">
          <input
            id="admin-reset-pw2"
            type="password"
            autoComplete="new-password"
