...
export function AdminDashboardWorkspace() {
  ...
  // Tenant-linked KPIs come from the live super-admin tenant list (same source
  // as /admin/tenants, so the numbers stay consistent). Everything else stays
  // on the dashboard mock until those modules are integrated.
  const { tenants: liveTenants } = useAdminTenants();
  const d = React.useMemo(() => {
    if (!liveTenants) return MOCK_ADMIN_DASHBOARD;
    const activeTenants = liveTenants.filter((t) => t.status === "active").length;
    return {
      ...MOCK_ADMIN_DASHBOARD,
      kpis: { ...MOCK_ADMIN_DASHBOARD.kpis, tenants: liveTenants.length },
      pipeline: { ...MOCK_ADMIN_DASHBOARD.pipeline, tenantsActive: activeTenants },
    };
  }, [liveTenants]);
