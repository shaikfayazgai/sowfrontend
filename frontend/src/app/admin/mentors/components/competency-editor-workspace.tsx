...
import { saveMentorCompetencyApi } from "@/lib/admin/mentors-api";
import { useAdminMentorDetail } from "@/lib/hooks/use-admin-mentors";
import { CompetencyEditorSkeleton } from "./competency-editor-skeleton";
...
export function CompetencyEditorWorkspace() {
  ...
  const { mentor, competency: seed, status: detailStatus } = useAdminMentorDetail(params.mentorId);
  ...
  if (detailStatus === "loading" && !mentor) return <CompetencyEditorSkeleton />;
  ...
  function handleSave() {
    ...
    void (async () => {
      try {
        await saveMentorCompetencyApi(params.mentorId, rows);
        router.push(`/admin/mentors/${params.mentorId}?tab=competency&competency=saved`);
      } catch (err) {
        setError(err instanceof Error ? err.message : "Couldn't save competency. Please try again.");
        setSaving(false);
      }
    })();
  }
