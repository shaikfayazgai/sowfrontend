...
/** 5–6 char lowercase alphanumeric suffix that makes a tenant ID unique. */
function randomSuffix(): string {
  const alphabet = "abcdefghijklmnopqrstuvwxyz0123456789";
  const len = 5 + Math.floor(Math.random() * 2); // 5 or 6
  let s = "";
  for (let i = 0; i < len; i++) s += alphabet[Math.floor(Math.random() * alphabet.length)];
  return s;
}

/** Tenant ID = slugified name + "-" + random suffix (unique by construction). */
function buildTenantId(name: string, suffix: string): string {
...
export function NewTenantWorkspace() {
  ...
  // Uniqueness check uses ONLY the live backend tenant slugs (the source of
  // truth). The persisted local store is intentionally excluded — it accumulates
  // stale test tenants and caused false "ID already taken" warnings. A truly
  // duplicate slug is still rejected by the backend (409) on submit.
  const { tenants: liveTenants } = useAdminTenants();
  const dynamicSlugs = React.useMemo(
    () =>
      (liveTenants ?? []).map(
        (t) => (t as { slug?: string }).slug ?? t.id.replace(/^tnt-/, ""),
      ),
    [liveTenants],
  );
  ...
  // Stable random suffix for the auto-generated tenant ID (name-<suffix>).
  const suffixRef = React.useRef<string>(randomSuffix());
  ...
  // Uniqueness is NOT a live blocker anymore — the tenant ID is auto-generated
  // with a random suffix and re-validated (regenerated if taken) on Continue.
  const blockers = stepBlockers(step, d);
  ...
  function onChange<K extends keyof TenantDraft>(k: K, v: TenantDraft[K]) {
    setD((p) => {
      const next = { ...p, [k]: v };
      // Auto tenant ID = slugified name + stable random suffix (unique by design).
      if (k === "name" && !slugTouched) next.slug = buildTenantId(String(v), suffixRef.current);
      return next;
    });
  }
  ...
  function goNext() {
    ...
    let draft = d;
    // On leaving Tenant Info, ensure the tenant ID is unique. If it's already
    // taken (manual edit, or a rare suffix collision), regenerate the random
    // suffix until it's free — so the user never hits a duplicate on submit.
    if (step === 0) {
      let slug = d.slug;
      let tries = 0;
      while ((!slug || slugStatusHint(slug, dynamicSlugs).taken) && tries < 8) {
        suffixRef.current = randomSuffix();
        slug = buildTenantId(d.name, suffixRef.current);
        tries += 1;
      }
      if (slug !== d.slug) {
        draft = { ...d, slug };
  ...
  async function onProvision() {
    ...
    // Create the tenant in the backend (also provisions the first admin with a
    // default password, emails the credentials, and flags forced first-login
    // reset). The full 6-step wizard draft maps directly to the backend payload.
    try {
      const res = await fetch("/api/superadmin/tenants", {
        ...
        body: JSON.stringify({
          ...
          currency: d.currency,
          timezone: d.timezone,
          retentionAudit: d.retentionAudit,
          retentionEvidence: d.retentionEvidence,
          residencyRegion: d.residencyRegion,
          consentVersion: d.consentVersion,
        }),
      });
      if (!res.ok) {
        // Surface the real backend error instead of falsely showing "provisioned".
        const err = (await res.json().catch(() => ({}))) as { detail?: string | { message?: string } };
        const msg =
          typeof err.detail === "string"
            ? err.detail
            : err.detail?.message ?? `Couldn't create the tenant (error ${res.status}). Please try again.`;
        setProvisionError(msg);
        setProvisioning(false);
        return;
      }
    } catch {
      setProvisionError("Couldn't reach the server. Please try again.");
      setProvisioning(false);
      return;
    }
    // Backend created it + emailed credentials. Keep a local provisioning-status
    // record so the provisioning page renders, then navigate.
      ...
      <div className="grid gap-5 lg:grid-cols-[264px_1fr] items-start">
        ...
        <div className={cn(DASH_CARD, "overflow-hidden")}>
          ...
          <div className="px-5 sm:px-6 py-5">
            ...
            {isLast && provisionError && (
              <div
                role="alert"
                className="mt-5 flex items-start gap-2 rounded-lg px-3.5 py-2.5"
                style={{ background: TONE.error.soft, border: `1px solid ${TONE.error.border}` }}
              >
                <Info className="h-4 w-4 shrink-0 mt-0.5" strokeWidth={2.2} style={{ color: TONE.error.text }} aria-hidden />
          ...
          <footer className="px-5 sm:px-6 py-4 border-t border-stroke-subtle flex flex-wrap items-center justify-between gap-3">
            ...
            {!isLast ? (
              ...
              <button type="button" disabled={provisioning} onClick={onProvision} className={cn(primaryBtnClass, "px-5")} style={primaryStyle}>
                ...
                {provisioning ? "Sending…" : "Send onboarding"}
              </button>
            )}
          </footer>
        </div>
...
function stepBlockers(step: number, d: TenantDraft): string[] {
  switch (step) {
    case 0: {
      const b: string[] = [];
      if (d.name.trim().length <= 1) b.push("tenant name");
      if (d.slug.trim().length <= 1) b.push("tenant ID");
      // Uniqueness is enforced/auto-fixed on Continue (random suffix), not here.
