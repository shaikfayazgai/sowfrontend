...
import { Building2, ChevronRight, FileText, Loader2, Plus, Search, SearchX, Users, X } from "lucide-react";
import {
...
export function TenantsWorkspace() {
  ...
  // Live tenants from the backend (super-admin API) — the ONLY source of truth.
  // No mock fallback: while loading we show the skeleton, never mock data.
  const { tenants: liveTenants, loading: tenantsLoading, error: tenantsError, refresh: refreshTenants } = useAdminTenants();
  ...
  const counts = React.useMemo(
    () => ({
      ...
      paused: allTenants.filter((t) => t.status === "paused" || t.status === "suspended").length,
    }),
    [allTenants],
  );
  ...
  const filtered = React.useMemo(() => {
    ...
    let rows = allTenants.filter(
      (t) =>
        filter === "all" ||
        t.status === filter ||
        (filter === "paused" && t.status === "suspended"),
    );
    if (needle) {
      rows = rows.filter((t) => {
  ...
  // Skeleton until both the store is hydrated AND the first live fetch resolves
  // — prevents any flash of stale/mock data on refresh.
  if (!hydrated || (tenantsLoading && liveTenants === null)) return <TenantsSkeleton />;
  ...
  return (
    <div className="space-y-5 pb-4 animate-fade-in">
      ...
      <div className={cn(DASH_CARD, "overflow-hidden")}>
        ...
        {tenantsError && liveTenants === null ? (
          // Backend unreachable — don't falsely claim "no tenants" (data may exist).
          <TenantEmptyState
            icon={SearchX}
            title="Couldn't load tenants"
            description="The tenant service is unavailable right now. Your tenants are safe — this is a connection issue. Try again in a moment."
            action={
              <button type="button" onClick={refreshTenants} className={primaryBtnClass} style={primaryStyle}>
                Retry
              </button>
            }
          />
