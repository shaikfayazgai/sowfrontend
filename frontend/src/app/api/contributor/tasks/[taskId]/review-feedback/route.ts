...
/* ── GET /api/contributor/tasks/[taskId]/review-feedback ───────────────────── */
...
export async function GET(
  ...
  const url     = `${BACKEND}/api/contributor/tasks/${encodeURIComponent(taskId)}/review-feedback`;
  ...
  /*
   * 3 ─ Backend 404 (no review yet for this task / demo task).
   *     Return 404 so the UI can show "No review feedback available yet".
   */
  if (backendRes.status === 404) {
    return NextResponse.json(
      { detail: "No review feedback found for this task." },
      { status: 404 },
    );
  }
