...
/**
 * Mentor forced first-login password reset — same pattern as the enterprise one.
 *
 * Reached when a freshly-provisioned mentor (default password) signs in: the
 * backend flags it `must_change_password` → `session.user.requiresPasswordChange`.
 * The mentor layout bounces every route here until a new password is set.
 *
 * Flow: set new password → POST /api/mentor/auth/change-password (backend clears
 * the flag) → re-authenticate with the new password → mentor dashboard.
 */
...
export default function MentorResetPasswordPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      router.replace("/mentor/login?reason=password_changed");
    } catch {
  ...
  return (
    <LoginShell>
      ...
      <form onSubmit={onSubmit} className="space-y-4">
        <AuthField label="New password" htmlFor="mentor-reset-pw">
        ...
        <AuthField label="Confirm password" htmlFor="mentor-reset-pw2">
          <input
            id="mentor-reset-pw2"
            type="password"
            autoComplete="new-password"
      ...
      <p className="mt-6 pt-5 border-t border-stroke-subtle text-center font-body text-[13px] text-text-secondary">
        <button
          ...
          onClick={() => {
            clearClientSession();
            void signOut({ callbackUrl: "/mentor/login" });
          }}
