...
import { setMemberRoles } from "@/lib/settings/use-tenant-team";
...
export function MemberRolesDrawer({
  ...
  const onSubmit = async () => {
    ...
    setSaving(true);
    const ok = await setMemberRoles(member.id, [...selectedRoles]);
    setSaving(false);
    if (!ok) {
      setSubmitError("Couldn't save roles. Please try again.");
      return;
    }
    toast.success(
      ...
      footer={
        <div className="flex items-center justify-end gap-2 w-full">
          ...
          <button
            ...
            disabled={saving}
            style={primaryStyle}
            className={cn(primaryBtnClass, saving && "opacity-60 pointer-events-none")}
          >
