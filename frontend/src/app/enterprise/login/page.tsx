...
function EnterpriseLoginScreen() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // This door is for the workspace ADMIN only. Team members (PMO, Finance,
      // Security/IT, Legal, Reviewer, …) must sign in at /enterprise/users/login.
      // The tenant owner ('enterprise') + ent.admin members surface the "admin"
      // role on /me; everyone else is a member.
      const meRes = await fetch("/api/me", { cache: "no-store" });
      const me = (meRes.ok ? await meRes.json().catch(() => null) : null) as
        | { roles?: { code?: string }[] }
        | null;
      const isAdmin =
        role === "admin" ||
        role === "super_admin" ||
        (me
          ? (me.roles ?? []).some((r) => (r.code ?? "").replace(/^ent\./, "") === "admin")
          : true); // /me unavailable → don't lock the admin out on a transient blip
      if (!isAdmin) {
        await signOut({ redirect: false });
        setError(
          "This sign-in is for the workspace admin. Team members should use the Users sign-in below.",
        );
        setSubmitting(false);
        return;
      }
      ...
      // ONE door for every tenant user. We do NOT route by a "primary" role
      // (PMO → dashboard, reviewer → reviewer area, …) — any enterprise role signs
      // in here and lands on the shared workspace home. Roles only gate what's
      // available *inside* the workspace (the sidebar exposes role-specific areas
      // like QA Review). /enterprise/dashboard is open to all enterprise roles.
      router.push("/enterprise/dashboard");
    } catch {
      // Network blip / backend momentarily unreachable — never leave the button
      // stuck on "Working…"; surface a retryable error instead.
    ...
    <LoginShell>
      <header className="mb-6">
        ...
        <p className="mt-2 font-body text-[14px] text-text-secondary">
          Admin access to your GlimmoraTeam enterprise workspace
        </p>
      </header>
      ...
      <p className="mt-6 pt-5 border-t border-stroke-subtle font-body text-[12px] text-text-tertiary text-center leading-relaxed">
        Team member (PMO, Finance, Reviewer, …)?{" "}
        <Link
          href="/enterprise/users/login"
