...
interface AccountMenuProps {
  ...
  /** Where to land after sign-out (defaults to the main login). */
  signOutTo?: string;
}
        ...
        >
          ...
          <div className="border-t border-stroke-subtle py-1">
            <MenuButton
              ...
              onClick={() => {
                setOpen(false);
                // Clear all client session state (localStorage/cookies/cache)
                // BEFORE the redirect, then let NextAuth clear the server cookie.
                clearClientSession();
                signOut({ callbackUrl: signOutTo });
              }}
            />
          </div>
        </div>
