...
export const AccountMenu: React.FC<AccountMenuProps> = ({
  ...
  // Sign out: wipe all client state first (so a guest can't see cached data),
  // then return to the portal's own login. Enterprise users land on
  // /enterprise/login; everyone else on the shared /auth/login.
  const handleSignOut = React.useCallback(() => {
    setOpen(false);
    const loginUrl = pathname?.startsWith("/enterprise")
      ? "/enterprise/login"
      : "/auth/login";
    clearClientSession();
