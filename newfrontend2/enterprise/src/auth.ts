...
export const { handlers, signIn, signOut, auth } = NextAuth({
  ...
  events: {
    ...
    async signOut(message) {
      ...
      if (!sessionId) return;
      // Fire-and-forget: don't block the logout response on the Postgres
      // revocation write. NextAuth clears the session cookie immediately, so
      // the user is signed out instantly; the durable-row revocation completes
      // in the background (best-effort).
      void revokeSession(sessionId, "user_logout").catch((err) => {
        // eslint-disable-next-line no-console
