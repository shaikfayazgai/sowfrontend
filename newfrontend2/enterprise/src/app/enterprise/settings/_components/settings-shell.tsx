...
import { ArrowRight, Building2, CreditCard, Plug, ScrollText, ShieldCheck, Users } from "lucide-react";
...
export function SettingsShell({ children }: { children: React.ReactNode }) {
  ...
  const { status, rolesResolving, canAccessSettings, accessibleSections, sectionAccess } =
  ...
  React.useEffect(() => {
    // Don't act until both the session AND the role source (/api/me) have
    // resolved — otherwise we'd redirect on a transiently-empty role set.
    if (status === "loading" || rolesResolving) return;
    if (isIndex) return;
  ...
  }, [status, rolesResolving, isIndex, canAccessSettings, activeSection, sectionAccess, router]);

  if (status === "loading" || rolesResolving) {
