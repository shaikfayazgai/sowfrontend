...
/**
 * Enterprise forgot password — OTP-based reset (NO email reset-link).
 *
 * Scoped to the enterprise portal. Same three-step OTP flow as the platform
 * reset, but every navigation link returns to /enterprise/login.
 *   Step 1 (email):    enter email → backend emails a 6-digit OTP
 *   Step 2 (code):     enter the 6-digit OTP (verified against the backend)
 *   Step 3 (password): set a new password (POST /password/setup-after-otp)
 */
...
export default function EnterpriseForgotPasswordPage() {
  ...
  async function sendOtp(e?: React.FormEvent) {
    ...
    try {
      ...
      setStep("code");
      // Security: never reveal whether an account exists for this email — the
      // wording is identical regardless. The OTP expires in 5 minutes.
      setInfo(
        `If an account exists for ${email}, a 6-digit code is on its way. It expires in 5 minutes.`,
      );
    } catch {
      setError("Sending isn't working right now. Please try again in a moment.");
    } finally {
      setSubmitting(false);
    }
  }
  ...
  async function verifyOtp(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok || !body.verified) {
        // Wrong code, or it expired (codes are valid for 5 minutes).
      ...
      setError("Verifying isn't working right now. Please try again in a moment.");
    } finally {
      setSubmitting(false);
    }
  }
      ...
      footer={
        step === "done" ? undefined : (
          <>
            ...
            <AuthHeaderLink href="/enterprise/login">Sign in</AuthHeaderLink>
          </>
        )
      }
    >
      ...
      {step === "email" && (
        <form onSubmit={sendOtp} className="space-y-4">
          <AuthField label="Email" htmlFor="ent-fp-email">
            <input
              id="ent-fp-email"
              type="email"
              autoComplete="email"
          ...
          <AuthBackLink href="/enterprise/login">
      ...
      {step === "code" && (
        <form onSubmit={verifyOtp} className="space-y-4">
          <AuthField label="Verification code" htmlFor="ent-fp-code">
            <input
              id="ent-fp-code"
              inputMode="numeric"
      ...
      {step === "password" && (
        <form onSubmit={setNewPassword} className="space-y-4">
          <AuthField label="New password" htmlFor="ent-fp-pw">
            <input
              id="ent-fp-pw"
              type="password"
          ...
          <AuthField label="Confirm password" htmlFor="ent-fp-pw2">
            <input
              id="ent-fp-pw2"
              type="password"
              autoComplete="new-password"
