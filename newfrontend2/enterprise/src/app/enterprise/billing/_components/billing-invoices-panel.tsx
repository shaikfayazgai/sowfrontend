...
/**
 * Billing invoices panel — registry table (platform pattern).
...
import { listInvoicesMock, type InvoiceStatus, type InvoiceSummary } from "@/lib/billing/invoices-mock";
...
const STATUS_LABEL: Record<InvoiceStatus, string> = { paid: "Paid", overdue: "Overdue", pending: "Pending" };
...
function statusRank(status: InvoiceStatus): number {
  return status === "overdue" ? 3 : status === "pending" ? 2 : status === "paid" ? 1 : 0;
}
...
export function BillingInvoicesPanel() {
  ...
  const activeFilter: FilterKey = (searchParams.get("inv") as FilterKey | null) ?? "all";
  ...
  const sorted = React.useMemo(() => {
    ...
    const rows = invoices.filter((inv) => {
      ...
      if (needle && !`${inv.id} ${inv.project}`.toLowerCase().includes(needle)) return false;
      return true;
    });
  ...
  const setParam = React.useCallback(
    (changes: Record<string, string | null>) => {
      ...
      router.replace(qs ? `/enterprise/billing?${qs}` : "/enterprise/billing", { scroll: false });
    },
    [router, searchParams],
  );
      ...
      <div className="px-4 sm:px-5 pt-4 border-b border-stroke-subtle">
        <div className="flex flex-wrap items-start justify-between gap-3 pb-3">
          <div className="min-w-0">
            <h2 className="font-display text-[14px] font-bold tracking-[-0.01em] text-foreground">Invoices</h2>
            <p className="mt-0.5 font-body text-[12px] text-text-tertiary">
              {sorted.length} invoice{sorted.length === 1 ? "" : "s"}
              {counts.overdue > 0 && activeFilter === "all" ? <span className="text-error-text font-medium"> · {counts.overdue} overdue</span> : null} ·{" "}
              <Link href="/enterprise/billing/invoices" className="font-semibold text-text-link hover:underline underline-offset-2">
                Full ledger
              </Link>
            </p>
          </div>
          ...
          {TABS.map((tab) => {
            ...
            return (
              ...
              >
                ...
                {warn && !active ? <span aria-hidden className="h-1.5 w-1.5 rounded-full bg-[var(--color-error-solid)] shrink-0" /> : null}
              </button>
            );
          })}
        </nav>
            ...
            <table className="w-full min-w-[640px] border-collapse">
              <thead>
                <tr className="border-b border-stroke-subtle bg-bg-subtle/50">
                  <th className="px-4 sm:px-5 py-2.5 text-left font-body text-[10.5px] font-semibold uppercase tracking-[0.06em] text-text-tertiary">Invoice</th>
                  <th className="px-3 py-2.5 text-left font-body text-[10.5px] font-semibold uppercase tracking-[0.06em] text-text-tertiary">Issued</th>
              ...
              <tbody>
                {pageRows.map((inv) => (
                  <InvoiceRow key={inv.id} inv={inv} onOpen={() => router.push(`/enterprise/billing/invoices/${inv.id}`)} />
                ))}
              </tbody>
            </table>
          </div>
  ...
  return (
    <tr onClick={onOpen} className="group border-b border-stroke-subtle last:border-0 cursor-pointer hover:bg-bg-subtle/60 transition-colors">
      <td className="px-4 sm:px-5 py-3.5">
        <Link href={href} onClick={(e) => e.stopPropagation()} className="font-body text-[13.5px] font-semibold text-foreground hover:text-text-link truncate block max-w-[260px]" title={inv.project}>
          {inv.project}
        </Link>
        <span className="font-mono text-[11px] text-text-tertiary tabular-nums">{inv.id}</span>
      </td>
      <td className="px-3 py-3.5 font-body text-[12.5px] text-text-secondary tabular-nums whitespace-nowrap">{fmtDate(inv.issuedAt)}</td>
      <td className={cn("px-3 py-3.5 font-mono text-[12.5px] tabular-nums text-right whitespace-nowrap", overdue ? "text-error-text font-semibold" : "text-foreground")}>{fmtINR(inv.amountMinor)}</td>
      <td className="px-3 py-3.5">
        <Chip tone={statusTone(inv.status)} dot={false}>{STATUS_LABEL[inv.status]}</Chip>
      </td>
