/**
 * POST /api/auth/register/reviewer — self-register with invite code (Option 2).
 */

import { NextRequest, NextResponse } from "next/server";
import { z } from "zod";
import { registerReviewerFromInvite } from "@/lib/reviewer/register-reviewer";
...
export async function POST(req: NextRequest) {
  ...
  try {
    const result = await registerReviewerFromInvite(body);
    return NextResponse.json({ ok: true, userId: result.userId, email: result.email });
  } catch (err) {
    const message = err instanceof Error ? err.message : "Couldn't create your reviewer account.";
    const status =
