...
async def otp_send_email(body: OtpSendRequest, background_tasks: BackgroundTasks):
    ...
    code = generate_code()
    # Persist synchronously (Redis, ~ms) so verification works the instant the
    # caller has the code. The email itself is the slow part (Gmail SMTP ~5s), so
    # we hand it to a background task and return immediately — the endpoint no
    # longer blocks the UI on SMTP. Security-neutral: same {ok:true} response
    # whether or not the account exists, so it never reveals account existence.
    persisted = await store_otp("email", body.email, code)
    resp = {"ok": True, "message": "Code sent."}
    import os
    if not persisted:
        # Redis unavailable — we can't verify later, so surface the dev code.
        resp.update({"devFallback": True, "devOtp": code,
                     "message": "Redis unavailable — use the dev code."})
    else:
...
async def setup_after_otp(body: _SetupAfterOtp):
    if not body.email:
        raise HTTPException(status_code=422, detail="Email required")
    # The forgot-password UI verifies the code first (/otp/verify-email), and
    # verify_otp is single-use — it deletes the code on success. So by the time
    # this endpoint runs the code is usually already consumed. Accept EITHER a
    # still-valid code (single-call clients) OR a recent successful verification
    # marker (the two-step UI). Both paths require a genuine prior OTP.
