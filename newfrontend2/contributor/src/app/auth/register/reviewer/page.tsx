...
/**
 * Reviewer self-register — two-step UX (Option 2).
 * Step 1: Accept invitation (code, name, email)
 * Step 2: Create password (confirm + strength meter) → register + sign in
 */
...
function Inner() {
  ...
  async function onContinueStep1(e: React.FormEvent) {
    ...
    setValidating(true);
    try {
      const res = await fetch("/api/reviewers/invites/validate", {
        method: "POST",
      ...
      const body = (await res.json().catch(() => ({}))) as { message?: string; orgName?: string };
      if (!res.ok) {
  ...
  async function onSubmitPassword(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/auth/register/reviewer", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      if (!res.ok) {
        ...
        throw new Error(body.message ?? "Couldn't create your reviewer account.");
      }
      ...
      if (signed && !signed.error) {
        ...
        router.push("/auth/login?registered=reviewer");
      }
    } catch (err) {
      setError(err instanceof Error ? err.message : "Something went wrong.");
    } finally {
          ...
          <form onSubmit={onContinueStep1} className="space-y-3">
            ...
            <div>
              ...
              <p className="mt-1.5 font-body text-[11.5px] text-text-tertiary">
                Must match the address your admin invited.
              </p>
            </div>
  ...
  return (
    <AuthShell>
      <AuthCard
        ...
        subtitle={
          orgName
            ? `Last step for ${email} at ${orgName}. Choose a password you'll use to sign in.`
