...
/**
 * Watches the client session. The moment it goes unauthenticated (token expired
 * or signed out elsewhere), tear down client state and return to /reviewer/login.
 * Fires once — a ref guards against redirect loops.
 */
function ReviewerSessionGuard() {
  const { status } = useSession();
  const pathname = usePathname() ?? "";
  const firedRef = React.useRef(false);

  React.useEffect(() => {
    if (status === "unauthenticated" && !firedRef.current) {
      firedRef.current = true;
      void reviewerLogout(pathname);
    }
  }, [status, pathname]);
