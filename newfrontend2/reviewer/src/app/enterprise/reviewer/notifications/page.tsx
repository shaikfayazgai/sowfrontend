...
import { fetchReviewerNotifications } from "@/lib/api/reviewer-mock";
...
// Fallback shown only if the backend feed is unreachable.
...
export default function ReviewerNotificationsPage() {
  ...
  // Load the real feed; fall back to the sample set only if the backend fails.
  React.useEffect(() => {
    const ctrl = new AbortController();
    fetchReviewerNotifications(ctrl.signal)
      .then((res) => setItems(res.items))
      .catch(() => setItems(MOCK_NOTIFICATIONS))
        ...
        <div className="px-5 sm:px-6 py-5">
          {!loaded ? (
            <div className="py-10 text-center">
              <span className="inline-block h-5 w-5 rounded-full border-2 border-foreground/20 border-t-foreground animate-spin" aria-label="Loading" />
              <p className="mt-2 font-body text-[12px] text-text-tertiary">Loading notifications…</p>
            </div>
          ) : visible.length === 0 ? (
            <div className="py-10 text-center">
