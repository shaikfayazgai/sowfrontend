...
import { reviewerLogout } from "@/lib/reviewer/logout";
import { fetchReviewerProfile, type ReviewerProfile } from "@/lib/api/reviewer-mock";
...
export default function ReviewerProfilePage() {
  ...
  const [profile, setProfile] = React.useState<ReviewerProfile | null>(null);

  // Real reviewer identity (workspace/tenant, member-since, role label) from the backend.
  React.useEffect(() => {
    const ctrl = new AbortController();
    fetchReviewerProfile(ctrl.signal).then(setProfile).catch(() => setProfile(null));
    return () => ctrl.abort();
  }, []);
  ...
  const displayName = session?.user?.name ?? profile?.name ?? MOCK_REVIEWER_PROFILE.name;
  const email = session?.user?.email ?? profile?.email ?? MOCK_REVIEWER_PROFILE.email;
  const sessionCount = sessions?.length ?? 0;
  const title = profile?.title ?? MOCK_REVIEWER_PROFILE.title;
  const workspace = profile?.workspace && profile.workspace !== "—" ? profile.workspace : "Glimmora HQ";
  const roleLabel = profile?.roleLabel ?? "Enterprise Reviewer";
  const joinedAt = profile?.joinedAt ?? MOCK_REVIEWER_PROFILE.joinedAt;
    ...
    <div className="space-y-5 pb-12 animate-fade-in">
      ...
      <div className={cn(DASH_CARD, "p-5")}>
        <div className="flex flex-col sm:flex-row sm:items-start gap-4">
          ...
          <dl className="flex items-center gap-6 shrink-0 border-t sm:border-t-0 sm:border-l border-stroke-subtle pt-4 sm:pt-0 sm:pl-6">
            ...
            <QuickStat
              label="Member since"
              value={new Date(joinedAt).toLocaleDateString("en-GB", {
                month: "short",
                year: "numeric",
              })}
            />
      ...
      <div className={cn(DASH_CARD, "overflow-hidden")}>
        ...
        <dl className="divide-y divide-stroke-subtle">
          <IdentityRow label="Role" value={roleLabel} />
          <IdentityRow label="Workspace" value={workspace} />
          <IdentityRow
            label="Member since"
            value={new Date(joinedAt).toLocaleDateString("en-GB", {
              month: "short",
      ...
      <div className="space-y-5">
        ...
        {/* Sign out — clears local/session storage, caches, cookies + ends the session */}
        <section className={cn(DASH_CARD, "overflow-hidden")}>
          <div className="flex items-center justify-between gap-4 px-5 py-4">
            <div>
              <p className="font-body text-[13.5px] font-semibold text-foreground">Sign out</p>
              <p className="mt-0.5 font-body text-[12px] text-text-tertiary">
                Ends your session on this device and clears cached data.
              </p>
            </div>
            <button
              type="button"
              onClick={onSignOut}
              disabled={signingOut}
              className="inline-flex items-center gap-2 h-9 px-3.5 rounded-lg border border-error-border bg-error-subtle/40 font-body text-[13px] font-semibold text-error-text transition-colors hover:bg-error-subtle disabled:opacity-60 disabled:cursor-not-allowed"
            >
              {signingOut ? (
                <span className="h-3.5 w-3.5 rounded-full border-2 border-error-text/40 border-t-error-text animate-spin" aria-hidden />
              ) : (
