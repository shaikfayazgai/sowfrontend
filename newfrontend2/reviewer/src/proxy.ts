 ...
 *
 * The reviewer portal exposes its own dedicated, unauthenticated auth
 * surface (sign-in + OTP forgot-password) at `/reviewer/login` and
 * `/reviewer/forgot-password`. These two exact paths are public so a
 * not-yet-signed-in reviewer can reach them; the rest of the reviewer
 * portal lives under `/enterprise/reviewer/*` and stays role-gated.
 */
...
function loginRedirect(req: NextRequest, reason?: string): NextResponse {
  ...
  // Reviewer-portal paths bounce to the reviewer's own login (on logout or an
  // expired session); everyone else goes to the shared login.
  url.pathname = returnTo.startsWith("/enterprise/reviewer")
    ? "/reviewer/login"
    : "/auth/login";
  url.search = "";
