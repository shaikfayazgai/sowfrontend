...
# backend/.env lives two levels up from this file (backend/shared/config.py).
# Load it explicitly so the service picks up Neon / Redis / secret config even
# when launched directly with uvicorn (process env not pre-populated).
