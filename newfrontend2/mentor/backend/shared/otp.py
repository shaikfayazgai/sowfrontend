...
async def consume_verified(channel: str, identifier: str) -> None:
    """Clear the 'verified' marker so a completed flow (e.g. password reset)
    can't be replayed. No-op if it's already gone."""
    client = get_redis_client()
    if client is None:
        _mem_del(_verified_key(channel, identifier))
        return
    try:
        await client.delete(_verified_key(channel, identifier))
    except Exception as exc:  # noqa: BLE001
        logger.warning("[OTP] consume-verified failed: %s", exc)
