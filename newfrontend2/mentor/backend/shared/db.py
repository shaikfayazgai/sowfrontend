...
def _connect_with_retry(attempts: int = 5, connect_timeout: int = 10):
    ...
    Critical: TCP keepalives + statement_timeout. The handlers call psycopg2
    synchronously inside async routes, so a single hung query (e.g. SELECT 1 on a
    socket Neon idle-closed) would block the WHOLE event loop and make every
    endpoint — even /health — time out. Aggressive keepalives make the OS surface
    a dead connection in ~10-15s instead of hanging forever, and statement_timeout
    caps any single query server-side.
    """
    last_exc: Exception | None = None
    for i in range(attempts):
        try:
            # NOTE: no `options="-c statement_timeout=…"` — Neon's POOLED endpoint
            # (PgBouncer) rejects server settings in the startup packet. TCP
            # keepalives alone make the OS surface a dead/idle-closed socket fast
            # so SELECT 1 errors out (→ reconnect) instead of hanging the loop.
...
def get_redis_client() -> "Redis | None":
    ...
    if _redis_client is None:
        _redis_client = Redis.from_url(
            ...
            socket_timeout=3,
            # Keep the TLS socket alive and transparently reconnect when the
            # remote (RedisLabs) idle-closes it, so OTP/rate-limit ops stay fast
            # instead of stalling on a dead connection.
