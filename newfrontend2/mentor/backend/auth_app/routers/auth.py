...
from shared.otp import consume_verified, generate_code, is_recently_verified, rate_limit_ok, store_otp, verify_otp
...
async def otp_send_email(body: OtpSendRequest, background_tasks: BackgroundTasks):
    if not body.email:
    ...
    # If we can't even persist the code (no Redis), there's no point sending an
    # email — fall back to returning the dev code so local flows still work.
    if not persisted:
        return {"ok": True, "devFallback": True, "devOtp": code,
                "message": "Email/Redis unavailable — use the dev code."}

    # Send the email AFTER the response returns. SMTP to Gmail takes several
    # seconds; doing it inline made "Send code" feel slow. Starlette runs this
    # sync function in a threadpool, so the handler responds as soon as the OTP
    # is stored (~instant) and the email lands a moment later.
...
async def setup_after_otp(body: _SetupAfterOtp):
    if not body.email:
        raise HTTPException(status_code=422, detail="Email required")
    # The two-step UI verifies the code first (/otp/verify-email), which consumes
    # the single-use code and sets a short-lived "verified" marker. So here we
    # accept EITHER a still-valid code (single-step callers) OR a recent
    # verification (two-step forgot-password UI). Re-verifying the same code
    # would always fail because verify-email already deleted it.
    verified = await verify_otp("email", body.email, body.code) or \
        await is_recently_verified("email", body.email)
    if not verified:
        raise HTTPException(status_code=400, detail="Invalid or expired code")
    row = repo.find_account_by_email(body.email)
    if not row:
    ...
    # One-time use: clear the verified marker so the code/verification can't be
    # replayed to reset the password again.
    await consume_verified("email", body.email)
