...
export function buildMentorProfile(input: {
  ...
  role: MentorRole;
  /**
   * Durable profile fields read from the backend (mentor_profiles + settings).
   * Take precedence over the in-memory overrides and the persona template, so
  ...
  return {
    ...
    competency,
    // Precedence: durable backend value → in-memory override → persona template.
    bio: input.backend?.bio ?? overrides?.bio ?? template.bio,
    mentorshipIntro:
      input.backend?.mentorshipIntro ?? overrides?.mentorshipIntro ?? template.mentorshipIntro,
    languages:
      input.backend?.languages && input.backend.languages.length > 0
        ? input.backend.languages
        : overrides?.languages ?? template.languages,
    timezone: input.backend?.timezone ?? overrides?.timezone ?? template.timezone,
  };
}
