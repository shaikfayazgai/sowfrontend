  ...
  callbacks: {
    async jwt({ token, user, account }) {
      ...
      if (user) {
        ...
        // Normalize scoped mentor roles to the base portal role. The super-admin
        // provisions mentors as "mentor.senior" / "mentor.lead"; the mentor portal
        // only cares that the user is a mentor, so collapse any "mentor*" to
        // "mentor". Other roles pass through unchanged.
        {
          const rawRole = (u.role) || "contributor";
          token.role = (rawRole.startsWith("mentor") ? "mentor" : rawRole) as UserRole;
        }
