 ...
 *
 * `/mentor/login` is the portal-scoped sign-in page (mirrors the admin
 * login pattern). It must be reachable without a session, so it's exact-
 * matched here BEFORE the `/mentor` portal rule can bounce it to /auth/login.
 */
const PUBLIC_EXACT_PATHS = new Set<string>(["/", "/mentor/login"]);
...
function loginRedirect(req: NextRequest, reason?: string): NextResponse {
  ...
  // Portal-scoped login: a mentor whose session expired lands on /mentor/login
  // (which wipes client state on mount), not the generic /auth/login.
  url.pathname = req.nextUrl.pathname.startsWith("/mentor")
    ? "/mentor/login"
    : "/auth/login";
  url.search = "";
