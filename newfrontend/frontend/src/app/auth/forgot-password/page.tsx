...
function ForgotPasswordInner() {
  ...
  // Where to send the user back to sign in. Admins arrive with ?return=/admin/login
  // so the reset flow returns them to the admin login, not the main one.
  // Only allow internal paths (must start with a single "/").
  const rawReturn = searchParams.get("return") ?? "";
  const signInHref =
    rawReturn.startsWith("/") && !rawReturn.startsWith("//") ? rawReturn : "/auth/login";
  ...
  async function sendOtp(e?: React.FormEvent) {
    ...
    try {
      ...
      setStep("code");
      // Security: never reveal whether the account exists. The code is only
      // emailed when a matching account exists; the message is identical either way.
      setInfo(`If an account exists for ${email}, you'll receive a 6-digit code. It expires in 5 minutes.`);
    } catch {
  ...
  return (
    <AuthScreen
      ...
      footer={
        step === "done" ? undefined : (
          <>
            Remember your password? <AuthHeaderLink href={signInHref}>Sign in</AuthHeaderLink>
          </>
        )
      }
    >
      ...
      {step === "email" && (
        <form onSubmit={sendOtp} className="space-y-4">
          ...
          <AuthSubmitButton disabled={!email.includes("@") || submitting}>
            {submitting ? "Sending…" : "Send code"}
          </AuthSubmitButton>
          <AuthBackLink href={signInHref}>
      ...
      {step === "code" && (
        <form onSubmit={verifyOtp} className="space-y-4">
          ...
          <AuthSubmitButton disabled={code.trim().length < 4 || submitting}>
      ...
      {step === "password" && (
        <form onSubmit={setNewPassword} className="space-y-4">
          ...
          <AuthSubmitButton disabled={password.length < 8 || password !== confirm || submitting}>
