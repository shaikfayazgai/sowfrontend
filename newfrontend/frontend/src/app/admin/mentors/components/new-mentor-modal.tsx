...
import { inviteMentor } from "@/lib/admin/mentors-api";
...
interface NewMentorModalProps {
  open: boolean;
  onClose: () => void;
  /** Called after a mentor is successfully invited (so the list refreshes). */
  onCreated?: () => void;
}

export function NewMentorModal({ open, onClose, onCreated }: NewMentorModalProps) {
  ...
  function onSubmit(e: React.FormEvent) {
    ...
    void (async () => {
      ...
      setError(null);
      try {
        // Create the mentor in the backend: account + temp password emailed +
        // forced reset on first login. Email must be globally unique (409 if not).
